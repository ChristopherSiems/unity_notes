from asyncio import (DatagramProtocol, DatagramTransport, Future, TimeoutError,
                     create_task, gather, get_running_loop, run, wait_for)
...
class PeerListener(DatagramProtocol):
    ...
    def datagram_received(self, data, addr):
        create_task(self.handle_message(data, addr))

    async def handle_message(self, data, addr):
        ids = loads(data.decode("UTF-8"))["ids"]
        notes = []
...
async def peer_request(ip: str, ids: list[int], timeout: float) -> str | None:
    ...
    transport, _ = await loop.create_datagram_endpoint(
        lambda: PeerMessenger(
            str({"type": "peer_request", "ids": ids}), future),
        remote_addr=(ip, CLIENT_PORT),
    )
...
async def main():
    ...
    transport, _ = await loop.create_datagram_endpoint(
        lambda: PeerListener(future), local_addr=("0.0.0.0", CLIENT_PORT)
    )

    try:
        notes = await future  # waits for one incoming UDP packet
